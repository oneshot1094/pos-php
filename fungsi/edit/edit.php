...
if(!empty($_SESSION['admin'])){
	...
	if(!empty($_GET['supplier'])){
		$nama= htmlentities($_POST['supplier']);
		$alamat= htmlentities($_POST['alamat']);
		...
		$data[] = $telepon;
		$data[] = $id;
		$sql = 'UPDATE supplier SET  nama_supplier=?, alamat_supplier=?, telepon=? WHERE id_supplier=?';
		$row = $config -> prepare($sql);
		$row -> execute($data);
	...
	if(!empty($_GET['pelanggan'])){
		$nama= htmlentities($_POST['pelanggan']);
		$alamat= htmlentities($_POST['alamat']);
		$telepon= htmlentities($_POST['telepon']);
		$id= htmlentities($_POST['id']);
		$data[] = $nama;
		...
		$data[] = $telepon;
		$data[] = $id;
		$sql = 'UPDATE pelanggan SET  nama_pelanggan=?, alamat_pelanggan=?, telepon=? WHERE id_pelanggan=?';
		$row = $config -> prepare($sql);
		$row -> execute($data);
		echo '<script>window.location="../../index.php?page=pelanggan&uid='.$id.'&success-edit=edit-data"</script>';
	}
	...
	if(!empty($_GET['users'])){
		...
		$data[] = $nik;
		$data[] = $id;
		$sql = 'UPDATE member SET nm_member=?,alamat_member=?,telepon=?,email=?,gambar=?,NIK=? WHERE id_member=?';
		$row = $config -> prepare($sql);
		$row -> execute($data);
		...
		$sql = 'UPDATE login SET user=?,pass=md5(?),role=? WHERE id_member=?';
		$row = $config -> prepare($sql);
		$row -> execute($login);
		echo '<script>window.location="../../index.php?page=users&success-edit=edit-data"</script>';
	}
	...
	if(!empty($_GET['jual'])){
		...
		if($hasil['stok'] > $jumlah)
		{
			...
			$data1[] = $id_pelanggan;
			$data1[] = $id;
			$sql1 = 'UPDATE penjualan SET jumlah=?,total=?,id_pelanggan=? WHERE id_penjualan=?';
